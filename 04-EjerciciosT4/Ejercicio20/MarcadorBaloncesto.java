...
public class MarcadorBaloncesto {
    ...
    public void añadirCanasta(char equipo, int puntos) {
        ...
        if (puntos > 0 || puntos == 2 || puntos == 3) {
            if (equipo == 'L') {
                puntosLocal = puntos + puntosLocal;
            }
            if (equipo == 'V') {
                puntosVisitante = puntos + puntosVisitante;
            }
        } else {

            IllegalArgumentException e = new IllegalArgumentException("Se debe "
                    + "introducir 'L' o 'V'");
            throw e;
